...
const TRAITS = [
  { id:'parfait',     label:'Parfait',               emoji:'👑', pts:24, desc:"Égal à la somme de ses diviseurs propres. Il n'en existe que quatre sous 10 000.", test:n=>PARFAIT.has(n) },
  { id:'taxicab',     label:'Taxicab',               emoji:'🚕', pts:25, desc:"Le plus petit nombre exprimable de deux façons comme somme de deux cubes. Ramanujan l'a sorti depuis un lit d'hôpital, en parlant d'un taxi.", test:n=>n===1729 },
  { id:'kaprekarC',   label:'Constante de Kaprekar', emoji:'🌀', pts:25, desc:"Presque tout nombre à quatre chiffres finit ici si on le triture assez longtemps. Un trou noir arithmétique.", test:n=>n===6174 },
  { id:'fermat',      label:'Nombre de Fermat',      emoji:'🧿', pts:16, desc:"De la forme 2^(2^k)+1. Fermat pensait qu'ils étaient tous premiers. Fermat avait tort.", test:n=>FERMAT.has(n) },
  { id:'armstrong',   label:'Narcissique',           emoji:'💅', pts:14,  desc:"Égal à la somme de ses chiffres élevés à la puissance de leur nombre. Il se suffit à lui-même.", test:isArmstrong },
  { id:'vampire',     label:'Vampire',               emoji:'🧛', pts:13,  desc:"Produit de deux « crocs » formés de ses propres chiffres. 1260 = 21 × 60.", test:n=>VAMPIRE.has(n) },
  { id:'bizarre',     label:'Bizarre',               emoji:'👽', pts:13,  desc:"Abondant, mais jamais somme exacte d'un sous-ensemble de ses diviseurs. Le premier est 70. Personne ne sait s'il en existe d'impairs.", test:n=>BIZARRE.has(n) },
  { id:'mersenne',    label:'Premier de Mersenne',   emoji:'⛰️', pts:14,  desc:"Un premier de la forme 2^p − 1. Aristocratie absolue.", test:n=>MERSENNE.has(n) },
  { id:'amical',      label:'Amical',                emoji:'🤝', pts:11,  desc:"Il a une âme sœur : chacun est la somme des diviseurs propres de l'autre.", test:isAmicable },
  { id:'primorielle', label:'Primorielle',           emoji:'🧬', pts:10,  desc:"Produit de tous les premiers consécutifs à partir de 2.", test:n=>PRIMORIELLE.has(n) },
  { id:'hcn',         label:'Hautement composé',     emoji:'🏗️', pts:9,  desc:"Il a plus de diviseurs que tout nombre plus petit que lui. Un carrefour.", test:n=>HCN.has(n) },
  { id:'catalan',     label:'Catalan',               emoji:'🌳', pts:8,  desc:"Compte les façons de parenthéser, trianguler, arboriser. Il surgit partout en combinatoire.", test:n=>CATALAN.has(n) },
  { id:'factorielle', label:'Factorielle',           emoji:'❗', pts:8,  desc:"Le produit de tous les entiers jusqu'à un certain point. Croît de façon indécente.", test:n=>FACTORIELLE.has(n) },
  { id:'kaprekar',    label:'Kaprekar',              emoji:'✂️', pts:8,  desc:"Son carré peut être coupé en deux morceaux qui, additionnés, le reforment. 45² = 2025, 20 + 25 = 45.", test:isKaprekar },
  { id:'tronquable',  label:'Premier tronquable',    emoji:'🔪', pts:7,  desc:"Reste premier quand on lui arrache ses chiffres de droite, un par un.", test:isTruncatablePrime },
  { id:'pow2',        label:'Puissance de 2',        emoji:'💾', pts:4,  desc:"La monnaie officielle des ordinateurs.", test:isPow2 },
  { id:'fibo',        label:'Fibonacci',             emoji:'🐚', pts:4,  desc:"La somme des deux précédents. Les lapins, les tournesols, les spirales.", test:n=>FIBO.has(n) },
  { id:'cube',        label:'Cube parfait',          emoji:'🧊', pts:4,  desc:"Un entier au cube. Solide.", test:n=>n>1&&isCube(n) },
  { id:'repdigit',    label:'Repdigit',              emoji:'🔁', pts:4,  desc:"Le même chiffre, encore et encore. Hypnotique.", test:isRepdigit },
  { id:'emirp',       label:'Emirp',                 emoji:'🪞', pts:6,  desc:"Premier à l'endroit, premier à l'envers, et pas le même. (« prime » retourné.)", test:isEmirp },
  { id:'premier',     label:'Premier',               emoji:'🔷', pts:3,  desc:"Indivisible. Une brique de l'univers.", test:isPrime },
  { id:'carre',       label:'Carré parfait',         emoji:'⬛', pts:3,  desc:"Un entier au carré. Rassurant.", test:n=>n>1&&isSquare(n) },
  { id:'palindrome',  label:'Palindrome',            emoji:'🔄', pts:3,  desc:"Se lit pareil dans les deux sens.", test:isPalindrome },
  { id:'lucas',       label:'Lucas',                 emoji:'🌗', pts:3,  desc:"Le cousin discret de Fibonacci : même règle, autre départ.", test:n=>LUCAS.has(n) },
  { id:'smith',       label:'Smith',                 emoji:'🕴️', pts:3,  desc:"La somme de ses chiffres égale celle des chiffres de ses facteurs premiers.", test:isSmith },
  { id:'sophie',      label:'Sophie Germain',        emoji:'🎀', pts:2,  desc:"Un premier p tel que 2p+1 est premier aussi. Elle publiait sous un faux nom d'homme pour être lue.", test:isSophieGermain },
  { id:'triangle',    label:'Triangulaire',          emoji:'🔺', pts:2,  desc:"On peut l'empiler en triangle. 1, 3, 6, 10, 15…", test:n=>n>0&&isTriangular(n) },
  { id:'pentagonal',  label:'Pentagonal',            emoji:'⬟', pts:2,  desc:"Même idée, mais en pentagone.", test:isPentagonal },
  { id:'heureux',     label:'Heureux',               emoji:'😊', pts:2,  desc:"Additionnez le carré de ses chiffres, recommencez : vous finissez sur 1. Sinon vous bouclez à jamais sur 4.", test:isHappy },
  { id:'spheniq',     label:'Sphénique',             emoji:'🪨', pts:2,  desc:"Produit d'exactement trois premiers distincts.", test:n=>{const f=factorize(n);return f.length===3&&new Set(f).size===3;} },
  { id:'jumeau',      label:'Premier jumeau',        emoji:'👯', pts:1,  desc:"Un premier à distance 2 d'un autre premier. On ignore encore s'il y en a une infinité.", test:isTwinPrime },
  { id:'sexy',        label:'Premier sexy',          emoji:'💋', pts:1,  desc:"Vrai terme mathématique, désolé. Deux premiers séparés par 6 — « sex » signifie six en latin.", test:isSexyPrime },
  { id:'harshad',     label:'Harshad',               emoji:'🎁', pts:1,  desc:"Divisible par la somme de ses propres chiffres. « Joie » en sanskrit.", test:isHarshad },
  { id:'semipremier', label:'Semi-premier',          emoji:'🔗', pts:1,  desc:"Produit de deux premiers, exactement. Toute la cryptographie tient là-dessus.", test:n=>factorize(n).length===2 },
...
/* ---------- les nouveaux traits ---------- */
TRAITS.push(
  { id:'wieferich',  label:'Premier de Wieferich',   emoji:'🛸', pts:25, desc:"Un premier p tel que p² divise 2^(p−1) − 1. On n'en connaît que deux sous 10^17, et l'on ignore s'il en existe une infinité.", test:n=>WIEFERICH.has(n) },
  { id:'munchhausen',label:'Münchhausen',            emoji:'🎩', pts:26, desc:"Égal à la somme de ses chiffres élevés à eux-mêmes. Comme le baron, il se soulève par ses propres bottes. Il n'en existe que deux.", test:isMunchhausen },
  { id:'lychrel',    label:'Candidat de Lychrel',    emoji:'🔂', pts:18, desc:"Ajoutez-lui son miroir, recommencez : tout le monde finit palindrome, sauf lui. Plus d'un milliard d'itérations n'ont rien donné, et personne n'a su le démontrer.", test:n=>n===196 },
  { id:'carmichael', label:'Carmichael',             emoji:'🎭', pts:12, desc:"Composé, mais il réussit le test de primalité de Fermat pour toute base. Un menteur absolu — 1729 en fait partie.", test:n=>CARMICHAEL.has(n) },
  { id:'automorphe', label:'Automorphe',             emoji:'🪆', pts:10, desc:"Son carré se termine par lui-même. 76² = 5776.", test:isAutomorphe },
  { id:'dudeney',    label:'Dudeney',                emoji:'🧩', pts:10, desc:"Cube de la somme de ses propres chiffres. Six exemplaires connus, tous sous 20 000.", test:isDudeney },
  { id:'keith',      label:'Keith',                  emoji:'🪜', pts:9,  desc:"Amorcez une suite de Fibonacci avec ses chiffres : elle retombe exactement sur lui.", test:isKeith },
  { id:'idoneal',    label:"Idoine d'Euler",         emoji:'🗝️', pts:7,  desc:"Euler s'en servait pour repérer les grands premiers. Il en a trouvé 65 et personne n'en a jamais trouvé un 66ᵉ.", test:n=>IDONEAL.has(n) },
  { id:'achille',    label:'Achille',                emoji:'🦶', pts:6,  desc:"Puissant sans être une puissance : chaque facteur premier y est au carré au moins, et pourtant il n'est ni carré, ni cube, ni rien. Fort mais vulnérable.", test:isAchilles },
  { id:'permutable', label:'Premier permutable',     emoji:'🔀', pts:6,  desc:"Toutes les permutations de ses chiffres sont premières. 337, 373, 733 : tous premiers.", test:isPermutablePrime },
  { id:'bell',       label:'Bell',                   emoji:'🔔', pts:6,  desc:"Compte les façons de partitionner un ensemble. 5 objets se rangent de 52 manières.", test:n=>BELL.has(n) },
  { id:'motzkin',    label:'Motzkin',                emoji:'🎪', pts:5,  desc:"Compte les façons de relier des points sur un cercle par des cordes qui ne se croisent pas. Le cousin discret de Catalan.", test:n=>MOTZKIN.has(n) },
  { id:'gauche',     label:'Tronquable à gauche',    emoji:'🪓', pts:4,  desc:"Reste premier quand on lui arrache ses chiffres de gauche, un par un.", test:isLeftTruncatable },
  { id:'pell',       label:'Pell',                   emoji:'⛵', pts:4,  desc:"Chaque terme vaut deux fois le précédent plus celui d'avant. Ses rapports approchent la racine de deux.", test:n=>PELL.has(n) },
  { id:'pyramidal',  label:'Pyramidal carré',        emoji:'🗿', pts:3,  desc:"Un empilement de carrés. 4900 est le seul, hormis 1, à être lui-même un carré — le fameux problème des boulets de canon.", test:n=>PYRAMIDE.has(n) },
  { id:'tetraedrique',label:'Tétraédrique',          emoji:'🔻', pts:3,  desc:"Un empilement de triangles. La version en volume des nombres triangulaires.", test:n=>TETRA.has(n) },
  { id:'puissant',   label:'Puissant',               emoji:'💪', pts:3,  desc:"Si un premier le divise, son carré le divise aussi. Aucun facteur ne s'y promène seul.", test:isPowerful },
  { id:'chanceux',   label:'Chanceux',               emoji:'🍀', pts:3,  desc:"Survivant d'un crible qui raye un nombre sur deux, puis un sur trois, et ainsi de suite. Il en reste à peu près autant que de premiers.", test:n=>luckySet().has(n) },
  { id:'pratique',   label:'Pratique',               emoji:'🧰', pts:3,  desc:"Tout entier plus petit que lui s'écrit comme une somme de ses diviseurs. Fibonacci s'en servait pour décomposer les fractions.", test:isPractical },
  { id:'ondulant',   label:'Ondulant',               emoji:'〰️', pts:3,  desc:"Deux chiffres qui alternent sans fin. 1717, 2020, 45454.", test:isOndulant },
  { id:'hexagonal',  label:'Hexagonal',              emoji:'🔶', pts:2,  desc:"On peut l'empiler en hexagone. Tout hexagonal est aussi triangulaire.", test:isHexagonal },
  { id:'sur',        label:'Premier sûr',            emoji:'🛡️', pts:2,  desc:"Un premier p tel que (p−1)/2 soit premier aussi. Le miroir de Sophie Germain, et la clé de voûte de la cryptographie Diffie-Hellman.", test:isSafePrime },
  { id:'autonombre', label:'Auto-nombre',            emoji:'🪞', pts:2,  desc:"Aucun nombre, augmenté de la somme de ses chiffres, ne le produit. Il n'a pas de générateur : il ne descend de personne.", test:isAutoNombre },
  { id:'zuckerman',  label:'Zuckerman',              emoji:'✖️', pts:2,  desc:"Divisible par le produit de ses propres chiffres.", test:isZuckerman },
  { id:'cousin',     label:'Premier cousin',         emoji:'👪', pts:1,  desc:"Un premier à distance 4 d'un autre premier. Entre les jumeaux (2) et les sexy (6), il fallait bien quelqu'un.", test:isCousinPrime },
  { id:'pythagore',  label:'Premier de Pythagore',   emoji:'📐', pts:1,  desc:"Un premier de la forme 4k+1. Fermat a montré qu'ils sont exactement ceux qui s'écrivent comme somme de deux carrés.", test:isPythPrime },
...
const RARITIES = [
  ...
  { key:'peucommun', label:'Peu commun', min:3,  color:'#4ec97a', idx:1, coin:3,   dust:3,   forgeMult:1.5 },
  { key:'rare',      label:'Rare',       min:8,  color:'#4aa3ff', idx:2, coin:9,   dust:8,   forgeMult:3 },
  { key:'epique',    label:'Épique',     min:12, color:'#b567ff', idx:3, coin:26,  dust:22,  forgeMult:8 },
  { key:'legendaire',label:'Légendaire', min:18, color:'#ffb02e', idx:4, coin:70,  dust:60,  forgeMult:25 },
  { key:'mythique',  label:'Mythique',   min:27, color:'#ff4d6d', idx:5, coin:190, dust:160, forgeMult:60 },
];
